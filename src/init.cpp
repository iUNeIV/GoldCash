...
bool static Bind(CConnman& connman, const CService &addr, unsigned int flags) {
    if (!(flags & BF_EXPLICIT) && IsLimited(addr))
        return false;
    std::string strError;
    if (!connman.BindListenPort(addr, strError, (flags & BF_WHITGOLDST) != 0)) {
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-whitelistrelay", strprintf(_("Accept relayed transactions received from whitelisted peers even when not relaying transactions (default: %d)"), DEFAULT_WHITGOLDSTRELAY));
    strUsage += HelpMessageOpt("-whitelistforcerelay", strprintf(_("Force relay of transactions from whitelisted peers even if they violate local relay policy (default: %d)"), DEFAULT_WHITGOLDSTFORCERELAY));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/GoldCashcrypto/Gold-Cash>";
    const std::string URL_WEBSITE = "<https://goldcashcoin.co.uk>";
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-whitelistforcerelay", DEFAULT_WHITGOLDSTFORCERELAY)) {
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (fListen) {
        ...
        if (mapMultiArgs.count("-whitebind")) {
            BOOST_FOREACH(const std::string& strBind, mapMultiArgs.at("-whitebind")) {
                ...
                fBound |= Bind(connman, addrBind, (BF_EXPLICIT | BF_REPORT_ERROR | BF_WHITGOLDST));
            }
        }
    ...
    connman.AddNode("explorer.goldcashcrypto.co.uk");
    connman.AddNode("api.goldcashcrypto.co.uk");
    connman.AddNode("pool.goldcashcrypto.co.uk");
    connman.AddNode("node.goldcashcrypto.co.uk");
