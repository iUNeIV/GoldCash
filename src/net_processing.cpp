...
#if defined(NDEBUG)
# error "Goldcash cannot be compiled without assertions."
#endif
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        if (pfrom->fWhitelisted && GetBoolArg("-whitelistrelay", DEFAULT_WHITGOLDSTRELAY))
            fBlocksOnly = false;
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        if (!fRelayTxes && (!pfrom->fWhitelisted || !GetBoolArg("-whitelistrelay", DEFAULT_WHITGOLDSTRELAY)))
        {
        ...
        else if (fMissingInputs)
        {
            ...
            if (pfrom->fWhitelisted && GetBoolArg("-whitelistforcerelay", DEFAULT_WHITGOLDSTFORCERELAY)) {
...
bool SendMessages(CNode* pto, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            !(pto->fWhitelisted && GetBoolArg("-whitelistforcerelay", DEFAULT_WHITGOLDSTFORCERELAY))) {
